...
返 回 值 	: 	无
*************************************************/
void AS608_SendHead(uchar flag,uint length)
{
	/* 发送包头 */
	UART_SendByte(0xEF);
	UART_SendByte(0x01);
...
*************************************************/
//发送不带参数的命令包
void AS608_SendCmdPkg(uchar flag,uint length,uchar cmd)
{
	uint AS608_Bytes_Sum = (uint)flag+(uint)length+(uint)cmd;			//用以存储包标识至校验和之间所有字节之和(不包括校验和)
...
返 回 值 	: 	无
*************************************************/
void AS608_SendParamPkg(uchar flag,uint length,uchar cmd,uchar *data)
{
	uint AS608_Bytes_Sum = (uint)flag+(uint)length+(uint)cmd;			//用以存储包标识至校验和之间所有字节之和(不包括校验和)
	AS608_SendHead(flag,length);	//发送包的前段部分
	/* 发送指令码 */
...
返 回 值 	: 	无
*************************************************/
void AS608_SendDataPkg(uchar flag,uint length,uchar cmd,uchar *data)
{
	uint AS608_Bytes_Sum = (uint)flag+(uint)length;			//用以存储包标识至校验和之间所有字节之和(不包括校验和)
	AS608_SendHead(flag,length);	//发送包的前段部分
	/* 发送数据/参数 */
...
返 回 值 	: 	无
*************************************************/
void PS_Search(uchar BufferID,uint StartPage,uint PageNum)
{
	uchar  AS608_data_tmp[5];			//用以暂存 参数
...
返 回 值 	: 	无
*************************************************/
void PS_StoreChar(uchar BufferID,uint PageID)
{
	uchar  AS608_data_tmp[3];			//用以暂存 参数
	AS608_data_tmp[0]=BufferID;
...
返 回 值 	: 	无
*************************************************/
void PS_LoadChar(uchar BufferID,uint PageID)
{
	uchar  AS608_data_tmp[3];			//用以暂存 参数
	AS608_data_tmp[0]=BufferID;
	AS608_data_tmp[1]=PageID>>8;
...
返 回 值 	: 	无
*************************************************/
void PS_DownChar(uchar BufferID)
{
...
返 回 值 	: 	无
*************************************************/
void PS_DeletChar(uint PageID,uint N)
{
	uchar  AS608_data_tmp[4];			//用以暂存 参数
...
返 回 值 	: 	无
*************************************************/
void PS_WriteReg(uchar ID,uchar N)
{
	uchar  AS608_data_tmp[2];			//用以暂存 参数/数据
	AS608_data_tmp[0]=ID;
	AS608_data_tmp[1]=N;
...
返 回 值 	: 	无
*************************************************/
void PS_SetChipAddr(ulong Addr)
{
...
返 回 值 	: 	无
*************************************************/
void PS_Port_Control(uchar Cmd)
{
...
返 回 值 	: 	无
*************************************************/
void PS_WriteNotepad(uchar NotePageNum,uchar* content)
{
	uint AS608_Bytes_Sum = (uint)flag+(uint)length+(uint)cmd+(uint)NotePageNum;			//用以存储包标识至校验和之间所有字节之和(不包括校验和)
...
返 回 值 	: 	无
*************************************************/
void PS_GenBinImage(uchar BinImgTpye)
{
...
返 回 值 	: 	无
*************************************************/
void PS_UserGPIOCommand(uchar GPIOCx,uchar Status)
{
	uchar  AS608_data_tmp[5];			//用以暂存 参数
...
*************************************************/
    ...
    {0x20, "    地址错误    "}
		*/
void AS608_JudgeData(uchar *data)
{
	uint AS608_Bytes_Sum;	//存储应答包的校验和
	uint length;					//存储包长度
	uchar result;					//存储确认码
	/* 检验应答包格式 */
	if(*data==0xEF && *(data+1)==0x01 && *(data+2)==(uchar)(AS608Addr>>24) && *(data+3)==(uchar)(AS608Addr>>16) && *(data+4)==(uchar)(AS608Addr>>8) && *(data+5)==(uchar)(AS608Addr) && *(data+6)==0x07)
	{
		
		/* 确认是AS608应答包，开始检验校验和 */
		AS608_Bytes_Sum = *(data+7+length)<<8 + *(data+7+length+1);//获取校验和
		length = *(data+7)<<8 + *(data+8);	//获取包长度
		while(length--)	//开始检验
		{
			AS608_Bytes_Sum -= *(data+7+length);
		}
		if(AS608_Bytes_Sum)	/* AS608_Bytes_Sum!=0,即校验和不正确，检验失败 */
		{
			/* FAIL */	//自主配置，执行相应操作
		}else{	/* 校验和检测成功 */
